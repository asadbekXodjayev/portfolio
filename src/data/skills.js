/**
 * @typedef {Object} Skill
 * @property {string} name
 * @property {number} level  0-100 proficiency
 * @property {string} [colorKey]  optional GitHub-language name for a color dot (see lib/github LANG_COLORS)
 *
 * @typedef {Object} SkillGroup
 * @property {string} title
 * @property {Skill[]} items
 * @property {string} [variant]   TerminalWindow variant: 'phosphor-green' | 'cyan' | 'amber'
 * @property {boolean} [highlight] elevated styling (used for the AI/ML tier)
 * @property {string} [badge]      small badge label shown on the group header
 */

/** @type {SkillGroup[]} */
export const skillGroups = [
  {
    title: 'languages.sys',
    items: [
      { name: 'JavaScript', level: 98, colorKey: 'JavaScript' },
      { name: 'TypeScript', level: 92, colorKey: 'TypeScript' },
      { name: 'Python', level: 85, colorKey: 'Python' },
      { name: 'C++', level: 80, colorKey: 'C++' },
      { name: 'C#', level: 75, colorKey: 'C#' },
      { name: 'Dart', level: 70, colorKey: 'Dart' },
      { name: 'Kotlin', level: 70, colorKey: 'Kotlin' },
      { name: 'Swift', level: 70, colorKey: 'Swift' },
    ],
  },
  {
    title: 'frontend.sys',
    items: [
      { name: 'React', level: 100 },
      { name: 'Next.js', level: 100 },
      { name: 'Three.js', level: 100 },
      { name: 'Framer Motion', level: 100 },
      { name: 'styled-components / Sass', level: 100 },
      { name: 'Tailwind CSS', level: 100 },
      { name: 'HTML5 / CSS3', level: 100 },
      { name: 'Zustand', level: 100 },
      { name: 'shadcn/ui', level: 100 },
      { name: 'FSD', level: 100 },
    ],
  },
  {
    title: 'mobile.sys',
    items: [
      { name: 'React Native', level: 75 },
      { name: 'Flutter', level: 70 },
      { name: 'Kotlin', level: 70 },
      { name: 'Swift', level: 70 },
    ],
  },
  {
    title: 'ai-ml.sys',
    variant: 'cyan',
    highlight: true,
    badge: 'Machine Learning',
    items: [
      { name: 'Python (ML)', level: 82, colorKey: 'Python' },
      { name: 'PyTorch', level: 70 },
      { name: 'TensorFlow / Keras', level: 70 },
      { name: 'scikit-learn', level: 72 },
      { name: 'NumPy / Pandas', level: 80 },
      { name: 'Hugging Face', level: 70 },
      { name: 'OpenAI API', level: 85 },
      { name: 'LangChain', level: 70 },
      { name: 'Jupyter', level: 78 },
    ],
  },
  {
    title: 'ai-agents.sys',
    variant: 'amber',
    badge: 'Agentic Workflows',
    items: [
      { name: 'Claude (Anthropic)', level: 100 },
      { name: 'GPT / Codex (OpenAI)', level: 100 },
      { name: 'GitHub Copilot', level: 100 },
      { name: 'Gemini (Google)', level: 100 },
      { name: 'Cursor / Windsurf', level: 100 },
      { name: 'Grok (xAI)', level: 100 },
      { name: 'Groq (LPU inference)', level: 100 },
      { name: 'Qwen (Alibaba)', level: 100 },
      { name: 'Kimi (Moonshot AI)', level: 100 },
    ],
  },
  {
    title: 'apis.sys',
    items: [
      { name: 'OpenAI API', level: 85 },
      { name: 'Claude (Anthropic) API', level: 82 },
      { name: 'Telegram Bot API', level: 80 },
      { name: 'OpenAPI / Swagger', level: 78 },
      { name: 'GitHub REST API', level: 82 },
      { name: 'Spotify API', level: 74 },
      { name: 'OAuth 2.0', level: 85 },
    ],
  },
  {
    title: 'qa-automation.sys',
    items: [
      { name: 'Playwright', level: 78 },
      { name: 'Jest / Vitest', level: 80 },
      { name: 'Allure Report', level: 70 },
      { name: 'Cypress', level: 70 },
      { name: 'Postman / Newman', level: 75 },
      { name: 'AI-assisted test generation', level: 80 },
    ],
  },
  {
    title: 'backend.sys',
    items: [
      { name: 'Node.js', level: 70 },
      { name: 'Django', level: 70 },
      { name: 'FastAPI', level: 72 },
      { name: 'REST APIs', level: 85 },
      { name: 'SQL', level: 70 },
    ],
  },
  {
    title: 'tooling.sys',
    items: [
      { name: 'Git', level: 90 },
      { name: 'GitHub Actions (CI/CD)', level: 75 },
      { name: 'Docker / Docker Compose', level: 70 },
      { name: 'Vite / Webpack', level: 95 },
      { name: 'npm', level: 95 },
      { name: 'Figma', level: 90 },
    ],
  },
  {
    title: 'platforms.sys',
    items: [
      { name: 'Windows (native C++/C#)', level: 80 },
      { name: 'Linux', level: 70 },
      { name: 'Vercel / Netlify', level: 92 },
    ],
  },
];

// Top skills for the Home "skills" teaser tag row.
export const topSkills = [
  'React',
  'TypeScript',
  'Next.js',
  'Three.js',
  'Framer Motion',
  'Python',
  'PyTorch',
  'C++',
  'C#',
  'Claude API',
  'OpenAI API',
  'AI Agents',
  'Playwright',
  'CI/CD',
  'Node.js',
];
